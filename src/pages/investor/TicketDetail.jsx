...
import { ArrowLeft, Send, Clock, CheckCircle, AlertCircle, User, Shield } from "lucide-react";
...
export default function TicketDetail() {
  ...
  const fetch = useCallback(async () => {
    try {
      setLoading(true);
      const data = await ticketApi.getTicketDetail(id);
  ...
  const handleReply = async () => {
    ...
    setSending(true);
    try {
      await ticketApi.addReply(id, reply);
      setReply("");
      fetch();
    } catch (err) { /* silent */ }
  ...
  if (loading) {
    return <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-6"><Skeleton className="h-8 w-48" /><Skeleton.Card /><Skeleton.Card /></div>;
  }
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      ...
      <Card>
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
          ...
          {ticket.assignee && <span>Assigned: {ticket.assignee.firstName} {ticket.assignee.lastName}</span>}
        </div>
      ...
      <div className="space-y-4">
        {replies.map((r) => (
          <div key={r.id} className={`flex gap-3 ${r.adminId ? "justify-start" : "justify-start"}`}>
            ...
            <div className={`rounded-xl p-4 max-w-[80%] ${r.adminId ? "bg-neon-tangerine/10 border border-neon-tangerine/20" : "bg-gray-50 border border-gray-100"}`}>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-semibold text-gray-700">{r.adminId ? "Support Team" : "You"}</span>
      ...
      {ticket.status !== "closed" && (
        <Card>
          ...
          <textarea value={reply} onChange={(e) => setReply(e.target.value)} rows={3} placeholder="Type your message..."
