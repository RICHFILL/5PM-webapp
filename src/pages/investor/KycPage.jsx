      ...
      {stepLabels.map((label, i) => (
        <div key={label} className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          {i < stepLabels.length - 1 && (
            <div
              className={`w-3 sm:w-4 h-0.5 ${i < currentStep ? "bg-green-400" : "bg-gray-200"}`}
            />
          )}
        </div>
      ))}
      ...
      <div>
        ...
        <div className="flex gap-4">
          {["Male", "Female"].map((g) => (
            <label
              key={g}
              className="flex items-center gap-2 cursor-pointer px-4 py-2.5 border border-gray-300 rounded-xl has-[:checked]:border-neon-tangerine has-[:checked]:bg-neon-tangerine/5 transition-colors"
            >
              <input
                type="radio"
                name="gender"
...
function Step3Identity({ data, onChange, onNext, onPrev }) {
  ...
  const identityType = data.identityType || "bvn";
  const identityNumber = data.identityNumber || "";

  const handleTypeChange = (type) => {
    onChange({ ...data, identityType: type, identityNumber: "" });
    setErrors((p) => ({ ...p, identityNumber: "" }));
  };

  const handleNumberChange = (e) => {
    const v = e.target.value.replace(/\D/g, "").slice(0, 11);
    onChange({ ...data, identityType, identityNumber: v });
    setErrors((p) => ({ ...p, identityNumber: "" }));
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const errs = {};
    if (!identityNumber.trim() || identityNumber.length !== 11) {
      errs.identityNumber = `Enter a valid 11-digit ${identityType.toUpperCase()}`;
    }
    if (Object.keys(errs).length) {
      setErrors(errs);
      return;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
        ...
        <p>
          Your BVN or NIN is required for identity verification as mandated by
          regulatory requirements. This information is encrypted and securely
          stored.
        </p>
      </div>
          ...
          {["bvn", "nin"].map((t) => (
            ...
            >
              <input
                ...
                name="identityType"
                value={t}
                checked={identityType === t}
                onChange={() => handleTypeChange(t)}
                className="text-neon-tangerine focus:ring-neon-tangerine"
              />
              <span className="text-sm text-gray-700">{t.toUpperCase()}</span>
            </label>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-sm text-gray-600">
        Upload clear, legible copies of the following documents. Accepted
        formats: JPG, PNG, PDF.
      </p>
      {documentFields.map(({ key, label, accept, required }) => (
        <div
          key={key}
          className="border-2 border-dashed border-gray-300 rounded-xl p-4 md:p-6 text-center hover:border-neon-tangerine/60 transition-colors"
        >
          {data[key] ? (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="text-sm font-medium text-gray-900">
                    {data[key].name || "Uploaded file"}
                  </p>
                  <p className="text-xs text-gray-500">
                    {data[key].size
                      ? `${(data[key].size / 1024 / 1024).toFixed(2)} MB`
                      : ""}
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => onChange({ ...data, [key]: null })}
      ...
      <div className="flex justify-between pt-4 border-t border-gray-100">
        ...
        <Button type="button" onClick={onNext} disabled={!allDocsUploaded}>
          Continue <ChevronRight size={16} />
        </Button>
      </div>
      {!allDocsUploaded && (
...
function Step5Selfie({ data, onChange, onNext, onPrev }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-sm text-gray-600">
        Upload a clear photo of your face to complete identity verification.
      </p>
      {data ? (
        <div className="text-center">
          <img
            ...
            alt="Selfie"
            className="w-48 h-48 object-cover rounded-2xl mx-auto border-2 border-green-500 shadow-sm"
          />
    ...
    <div className="space-y-6">
      <div className="bg-neon-tangerine/10 border border-neon-tangerine/30 rounded-xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-neon-tangerine">
              Review Your Information
            </p>
            <p className="text-sm text-gray-600 mt-1">
              Please confirm all details before submitting. You will not be able
              to edit most fields after submission.
            </p>
          </div>
        </div>
      </div>
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Card className="p-4">
          ...
          <p className="text-sm text-gray-600">
            {identity.identityType?.toUpperCase() || "ID"}: ****
            {identity.identityNumber?.slice(-4)}
          </p>
        </Card>
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-sm text-gray-600">
            Passport: {docs.passport?.name ? "Uploaded" : "Missing"}
          </p>
          <p className="text-sm text-gray-600">
            ID: {docs.governmentId?.name ? "Uploaded" : "Missing"}
          </p>
          <p className="text-sm text-gray-600">
            Utility: {docs.utilityBill?.name ? "Uploaded" : "Missing"}
          </p>
          <p className="text-sm text-gray-600">
            Selfie: {store.selfie ? "Uploaded" : "Missing"}
          </p>
        </Card>
      </div>
      <div className="flex justify-between pt-4 border-t border-gray-100">
...
function KycStatusPage({ status, onStartOver }) {
  const statusConfig = {
    [KYC_STATUS.PENDING]: {
      ...
      title: "KYC Pending",
      message:
        "Your KYC has been submitted. We are reviewing your application.",
    },
    [KYC_STATUS.UNDER_REVIEW]: {
      icon: AlertCircle,
      variant: "info",
      title: "Under Review",
      message: "Your documents are being verified by our compliance team.",
    },
    [KYC_STATUS.APPROVED]: {
      icon: CheckCircle2,
      variant: "success",
      title: "KYC Approved",
      message: "Your KYC has been approved. You can now start investing.",
    },
    [KYC_STATUS.REJECTED]: {
      icon: XCircle,
      variant: "danger",
      title: "KYC Rejected",
      message:
        "Your KYC was not approved. Please resubmit with correct information.",
    },
  };
  ...
  return (
    <Card className="text-center py-12 px-6">
      ...
      {status === KYC_STATUS.REJECTED && (
        <div className="mt-6">
          <Button onClick={onStartOver}>Re-submit KYC</Button>
        </div>
      )}
    </Card>
  );
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          {store.status === KYC_STATUS.REJECTED
            ? "Resubmit your KYC with correct information"
      ...
      <Card>
        ...
        {store.currentStep === 1 && (
          <Step2Address
            data={store.addressInfo}
            onChange={store.setAddressInfo}
            onNext={store.nextStep}
        ...
        {store.currentStep === 2 && (
          <Step3Identity
            data={store.identityInfo}
            onChange={store.setIdentityInfo}
            onNext={store.nextStep}
            onPrev={store.prevStep}
          />
        )}
