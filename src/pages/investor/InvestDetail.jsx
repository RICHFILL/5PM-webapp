...
import { formatCurrency, formatNaira, formatUSD } from "../../utils/format";
        ...
        <div className="flex items-start justify-between mb-6">
          ...
          <div style={{ position: "fixed", top: 0, left: "-9999px" }}>
            <InvestmentCertificate
              ...
              investorName={
                investment.investorName || user?.firstName + " " + user?.lastName || investment.user?.fullName
              }
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-50 rounded-xl p-4">
            ...
            <p className="text-lg font-bold text-gray-900">
              {investment.currency === "USD" ? formatUSD(investment.amount) : formatNaira(investment.amount)}
            </p>
          </div>
          <div className="bg-slate-50 rounded-xl p-4">
            <div className="flex items-center gap-2 text-xs text-gray-500 mb-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <p className="text-xl font-bold text-gray-900">
            {formatCurrency(investment.interestEarned, investment.currency)}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-xs uppercase tracking-wide text-gray-500 mb-1">
          ...
          <p className="text-xl font-bold text-gray-900">
            {formatCurrency(investment.expectedMonthlyRepayment, investment.currency)}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <p className="text-xs uppercase tracking-wide text-gray-500 mb-1">
          ...
          <p className="text-xl font-bold text-gray-900">
            {formatCurrency(investment.payoutUponExpiration, investment.currency)}
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-sm min-w-[500px]">
              <thead>
                <tr className="border-b border-gray-100 text-xs text-gray-400 uppercase tracking-[0.18em]">
                  ...
                  <th className="text-left pb-3 font-semibold px-6">
                    Payment Date
                  </th>
                  <th className="text-left pb-3 font-semibold px-6">
                    Due Date
              ...
              <tbody>
                {payments.map((p, i) => (
                  ...
                  >
                    <td className="py-3 font-semibold text-gray-900 px-6">
                      {formatNaira(p.amount)}
                    </td>
                    <td className="py-3 text-gray-600 px-6">
